...
//
// Filename    : DW_apb_uart_bcm21.v
// Revision    : $Id: //dwh/DW_ocb/DW_apb_uart/amba_dev/src/DW_apb_uart_bcm21.v#8 $
// Author      : Doug Lee    2/20/05
// Description : DW_apb_uart_bcm21.v Verilog module for DWbb
//
...
module i_uart_DW_apb_uart_bcm21 (
    clk_d,
    rst_d_n,
    init_d_n,
    data_s,
...
`endif
  generate
    if (SVA_TYPE == 1) begin : GEN_SVATP_EQ_1
      DW_apb_uart_sva01 #(WIDTH, (F_SYNC_TYPE & 7)) P_SYNC_HS (.*);
    end
    if (SVA_TYPE == 2) begin : GEN_SVATP_EQ_2
      DW_apb_uart_sva05 #(WIDTH, (F_SYNC_TYPE & 7)) P_SYNC_GC (.*);
    end
  endgenerate
`endif // SYNTHESIS
`endif // DWC_BCM_SNPS_ASSERT_ON
