...
// Revision: $Id: //dwh/DW_ocb/DW_ahb/amba_dev/src/DW_ahb_mux.v#7 $ 
--
-- File :                       DW_ahb_mux.v
...
// Registered version of hmaster
  reg  [`LOG2_NUM_INT_MASTERS-1:0] r_ihmaster_data;
...
//
  ...
  begin : hmaster_data_PROC
    if (hresetn == 1'b0)
      r_ihmaster_data <= {`LOG2_NUM_INT_MASTERS{1'b0}};
    else begin
      if (hready == 1'b1)
        r_ihmaster_data <= hmaster[`LOG2_NUM_INT_MASTERS-1:0];
    end
  end

  assign ihmaster_data[3] = 1'b0;
  assign ihmaster_data[2] = 1'b0;
  assign ihmaster_data[1] = r_ihmaster_data[1];
  assign ihmaster_data[0] = r_ihmaster_data[0];

  assign hmaster_data = ihmaster_data;
  assign ihmaster     = hmaster;
